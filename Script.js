...
function cetakDokumen(jenisLaporan) {
...
/* Export report to PDF using jsPDF (renders the report container) */
function exportLaporanPDF(jenisLaporan) {
    const container = document.getElementById('printTargetContainer');
    if(!container) return;
    container.classList.remove('hidden');
    // Build same content as cetakDokumen but without printing
    container.innerHTML = `<div style="text-align: center; border-bottom: 2px solid #000; padding-bottom: 1rem; margin-bottom: 1.5rem;">
            <h2 style="font-size: 1.25rem; font-weight: 900; text-transform: uppercase;">LAPORAN RESMI PERPUSTAKAAN SEKOLAH</h2>
            <p style="font-size: 0.875rem; color: #374151;">Jenis Dokumen: Laporan Data ${jenisLaporan} | Tanggal Cetak: ${new Date().toLocaleDateString('id-ID')}</p>
            <p style="font-size: 0.75rem; color: #6b7280; font-family: monospace; margin-top: 0.25rem;">Penanggung Jawab Sistem: Rian Kurniadi, S.T.</p>
        </div>`;
    ...
    try {
        ...
        doc.html(container, {
            callback: function (doc) {
                doc.save(`Laporan-${jenisLaporan}.pdf`);
        ...
        alert('Ekspor PDF gagal: ' + err.message);
        container.classList.add('hidden');
    }
}

/* Export report to Excel/XLSX using SheetJS */
function exportLaporanExcel(jenisLaporan) {
    ...
    let aoa = [];
    if (jenisLaporan === 'Buku') {
        aoa.push(['Kode Buku','Judul','Pengarang','Stok','Tersedia']);
        dataBuku.forEach(b => aoa.push([b.kode, b.judul, b.pengarang, b.stok, b.tersedia]));
    } else if (jenisLaporan === 'Anggota') {
        aoa.push(['ID Anggota','Nama','Tipe','Kontak']);
        dataAnggota.forEach(a => aoa.push([a.id, a.nama, a.tipe, a.kontak]));
    } else if (jenisLaporan === 'Sirkulasi') {
        aoa.push(['ID Transaksi','Peminjam','Buku','Pinjam','Batas','Status']);
        dataSirkulasi.forEach(s => aoa.push([s.id, s.namaAnggota, s.judulBuku, s.tglPinjam, s.tglKembali, s.status]));
    }

    const ws = XLSX.utils.aoa_to_sheet(aoa);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, jenisLaporan);
    const wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    const blob = new Blob([wbout], { type: 'application/octet-stream' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `Laporan-${jenisLaporan}.xlsx`;
